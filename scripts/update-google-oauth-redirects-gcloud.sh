#!/bin/bash
# Google OAuth リダイレクトURI更新スクリプト（gcloud iam oauth-clients使用）
...
# gcloud iam oauth-clientsを使用してリダイレクトURIを更新
echo "📝 Google OAuth 2.0クライアントのリダイレクトURIを更新します..."
echo ""

# 現在の設定を取得
echo "📋 現在の設定を確認しています..."
CURRENT_CONFIG=$(gcloud iam oauth-clients describe "$CLIENT_ID" \
  --project=$PROJECT_ID \
  --location=global \
  --format=json 2>/dev/null || echo "")

if [ -z "$CURRENT_CONFIG" ]; then
    echo "⚠️  gcloudコマンドでOAuth 2.0クライアントが見つかりません。"
...
# リダイレクトURIを更新
echo "🔄 リダイレクトURIを更新しています..."
gcloud iam oauth-clients update "$CLIENT_ID" \
  --project=$PROJECT_ID \
  --location=global \
  --allowed-redirect-uris="${CLOUD_RUN_URL}/api/auth/callback/google,${CUSTOM_DOMAIN}/api/auth/callback/google,http://localhost:3000/api/auth/callback/google" \
  --allowed-javascript-origins="${CLOUD_RUN_URL},${CUSTOM_DOMAIN},http://localhost:3000" \
  2>&1

if [ $? -eq 0 ]; then
    echo ""
    echo "✅ リダイレクトURIの更新が完了しました"
    echo ""
    echo "📋 設定されたリダイレクトURI:"
    echo "   - $CLOUD_RUN_URL/api/auth/callback/google"
    echo "   - $CUSTOM_DOMAIN/api/auth/callback/google"
    ...
    echo ""
    echo "❌ リダイレクトURIの更新に失敗しました"
    echo "   手動でGoogle Cloud Consoleから設定してください"
    exit 1
fi
